...
class WebshopSettings(Document):
	...
	def validate_field_filters(filter_fields, enable_field_filters):
		...
		valid_fields = [
			df.fieldname for df in web_item_meta.fields if df.fieldtype in ["Link", "Table MultiSelect", "Check", "Select"]
		]
		...
		for row in filter_fields:
			if row.fieldname not in valid_fields:
				frappe.throw(
					_(
						"Filter Fields Row #{0}: Fieldname {1} must be of type 'Link', 'Table MultiSelect', 'Check' or 'Select'"
